...
## DEV-034 - 2026-09-01 - 新手引导强化原生模型提示

阶段和模式：Release correction + packaging validation
相关 ID：CR-008、P-007B、TEST-007
客户请求：将新手引导新增配置部分的重点文案改为“供应商是GPT或者OpenAI模型时，无需获取模型”，并使用暗红色显示。
工作内容：新增明确的重点消息变量，只对该条消息使用暗红色 `#8b2f2a`；同步 README 重点提示、CHANGELOG 和正式新手引导截图。
验证证据：Python 语法检查、118 项标准库测试和 `git diff --check` 通过；截图为 `820×500`，暗红提示无换行或越界；PyInstaller 6.20.0、Inno Setup 6.7.3 重建和隔离启动通过。
结果：界面、截图、文档和两个正式产物已同步，新产物哈希写入所有发布文档。
待处理 Git 操作：快进推送本次修正并移动 `v1.4.0` 到最终提交，然后替换现有 Release 的两个资产和说明。
