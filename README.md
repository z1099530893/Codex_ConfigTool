...
点击邮箱可以复制到剪贴板；点击项目地址可以在默认浏览器中打开 GitHub 仓库。

## 新手引导

软件启动并完成配置检查后，会显示三步新手引导：

1. 填写服务商提供的 API Key、Base URL 和 Model，Provider 显示名称填写服务商名称
2. 点击“保存配置”，软件会自动备份已有配置
3. 完全退出并重新打开 Codex，使新配置生效

勾选“下次不再提示”后，引导状态保存在 `%APPDATA%\CodexConfigTool\settings.json`，不会写入 Codex 的 `config.toml`。关闭提示后，仍可从“关于软件”窗口点击“新手引导”重新打开。
...
普通保存因此只修改 Provider 段内部的 `name`、`base_url` 和顶层 `model`，不重命名 Provider ID。当前配置无法直接编辑时，“保存配置”会先备份，再自动创建稳定的自定义模板；这属于初始化转换，不会猜测或重命名已有自定义 Provider。

如果当前配置是干净的内置 `openai` 默认状态，软件启动时会自动备份并创建可编辑模板。复杂或冲突配置不会被自动覆盖。
...
- 只缺少 `config.toml`：备份并保留已有的 `auth.json`，再创建可编辑模板
- 当前是干净的官方 `openai` 默认配置：自动备份原配置并创建模板
...
首次完全没有配置时，状态栏会显示“检测到首次使用，已创建可编辑配置”。存在旧文件并发生自动转换时，状态栏会显示原配置的备份位置，不需要用户确认或执行额外步骤。

官方登录模式记录在工具自己的 `%APPDATA%\CodexConfigTool\settings.json` 中。软件关闭或电脑重启后仍然有效，直到用户填写 API 配置并点击“保存配置”，或恢复一个可编辑的自定义配置。

## 自动创建自定义模板

主界面不再提供手动“创建自定义模板”按钮。首次初始化或当前配置无法直接编辑时，软件会在启动或用户点击“保存配置”后自动创建模板。存在旧文件时会先自动备份；没有旧文件时直接创建，不生成空备份。
...
1. 备份当前的 `auth.json` 和 `config.toml`
2. 删除这两个配置文件
3. 保留聊天记录、本地数据库、日志和已有备份
4. 进入持久化的官方登录模式
5. 提示用户关闭本工具并启动 Codex，按照 Codex 的提示登录

软件不会生成替代的 `auth.json` 或 `config.toml`，而是由 Codex 自己创建当前版本需要的官方配置。官方登录模式下再次打开本工具，也不会自动覆盖这些官方配置。以后填写 API 配置并点击“保存配置”时，软件会自动备份并创建自定义模板，同时退出官方登录模式。
...
备份最多保留 5 个。创建新备份后，如果备份数量超过 5 个，工具会自动删除最旧的备份。

以下操作在存在旧配置时都会自动备份，并在操作完成后显示备份结果：
